...
// Payment Status + Admin Messages + Happy Messages
// ============================================
...
class DeliveryBoyApp {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.otp) {
                    ...
                    this.showLoginStatus(`🔐 OTP: ${this.currentOtp}`, 'pending');
                }
        ...
        const checkInterval = setInterval(async () => {
            try {
                ...
                if (data.success && data.approved) {
                    ...
                    localStorage.setItem('deliveryBoyPhone', this.deliveryBoyPhone);
                    localStorage.setItem('deliveryBoyName', this.deliveryBoyName);
                    this.showLoginStatus('✅ Approved!', 'success');
    ...
    showMainApp() {
        ...
        this.blockCheckInterval = setInterval(() => this.checkBlockedStatus(), 5000);
        this.onlineStatusInterval = setInterval(() => this.updateOnlineStatus(), 5000);
        this.messageCheckInterval = setInterval(() => this.checkAdminMessages(), 10000);
    ...
    getPaymentInfo(orderId) {
        return this.allPayments.find(p => p[1] === orderId) || null;
    }
    ...
    async checkAdminMessages() {
        try {
            const response = await fetch(`${API_URL}?action=getDeliveryBoyMessages&phone=${this.deliveryBoyPhone}`);
            const data = await response.json();
            
            if (data.success && data.messages && data.messages.length > 0) {
                const latestMessage = data.messages[data.messages.length - 1];
                this.showAdminMessageBanner(latestMessage);
            }
        } catch (error) {
            // Silent fail - messages check
        }
    }

    // ============================================
    // 🆕 SHOW ADMIN MESSAGE BANNER
    // ============================================
    showAdminMessageBanner(message) {
        if (this.adminMessageBanner) {
            if (this.adminMessageTitle) {
                this.adminMessageTitle.textContent = message[0] || 'Admin Message';
            }
            if (this.adminMessageBody) {
                this.adminMessageBody.textContent = message[1] || '';
            }
    ...
    async showAdminMessages() {
        try {
            ...
            if (data.success && data.messages && data.messages.length > 0) {
                ...
                container.innerHTML = '<p style="text-align:center;color:#999;">📭 कोई messages नहीं</p>';
            }
            
            this.adminMessagesModal?.classList.remove('hidden');
        } catch (error) {
            console.log('⚠️ Messages error:', error);
        }
    }

    // ============================================
    // 🆕 SHOW HAPPY/FUNNY MESSAGE
    // ============================================
    showHappyMessage() {
        const randomIndex = Math.floor(Math.random() * HAPPY_MESSAGES.length);
        const message = HAPPY_MESSAGES[randomIndex];
        
        if (this.happyMessageToast && this.happyMessageText) {
            this.happyMessageText.textContent = message;
    ...
    displayOrders(orders) {
        ...
        orders.forEach(order => {
            ...
            const paymentStatus = paymentInfo ? paymentInfo[10] : 'N/A';
            const paymentMethod = paymentInfo ? paymentInfo[8] : 'N/A';
            const paymentTotal = paymentInfo ? paymentInfo[7] : '0';
            
            const paymentStatusClass = paymentStatus === 'Verified' ? 'verified' : paymentStatus === 'Cancelled' ? 'cancelled' : 'pending';
                ...
                if (order) {
                    body.innerHTML = `
                        ...
                        <div style="margin-bottom:10px;padding:10px;background:#FAFAFA;border-radius:8px;">
                            ...
                            Method: ${paymentInfo[8]}<br>
                            Amount: ₹${paymentInfo[7]}<br>
                            Status: ${paymentInfo[10]}
                        </div>
                        ` : ''}
    ...
    initMap() {
        ...
        L.marker([27.6667496, 77.7124673]).bindPopup('🏪 Quick Dukan').addTo(this.currentMap);
        if (this.currentLocation) {
            L.marker([this.currentLocation.lat, this.currentLocation.lng]).bindPopup('🛵 आप यहाँ हैं').addTo(this.currentMap);
        }
        ...
        try {
            ...
            if (data.success && data.orders) {
                ...
                if (order && order[10] && order[11]) {
                    ...
                    L.marker([customerLat, customerLng]).bindPopup('📍 Customer').addTo(this.currentMap);
                    const bounds = L.latLngBounds([27.6667496, 77.7124673], [customerLat, customerLng]);
