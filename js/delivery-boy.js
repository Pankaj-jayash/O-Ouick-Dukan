...
const API_URL = 'https://script.google.com/macros/s/AKfycbwTkldLOKHQRVmMFs-vNyQZGhG1Ef2IHKMLnE8YkA7yS2dyjymZ2KzlRXRuC5aQUjcC/exec';
...
class DeliveryBoyApp {
    ...
    // ============================================
    initDeviceId() {
        // पहले से saved device ID check करें
        let savedDeviceId = localStorage.getItem('deliveryBoyDeviceId');
        
        if (!savedDeviceId) {
            // नई device ID generate करें
    ...
    checkAutoLogin() {
        // अगर पहले से login है तो main app show करें
    ...
    async sendOTP() {
        ...
        // Admin को request भेजें (device ID के साथ)
    ...
    async checkLoginApproval() {
        const checkInterval = setInterval(async () => {
            try {
                ...
                console.log('📊 Approval check:', data);
    ...
    showMainApp() {
        ...
        // Stats update हर 30 second में
        this.statsInterval = setInterval(() => this.loadStats(), 30000);
        
        // Location tracking शुरू करें
    ...
    async logout() {
        ...
        // Google Sheets में logout time save करें
        ...
        // Location tracking बंद करें
        this.stopLocationTracking();
        
        // Stats interval बंद करें
                ...
                if (order) {
                    body.innerHTML = `
                        <div style="margin-bottom:10px;">
                            <strong>Order ID:</strong> ${order[0]}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Customer:</strong> ${order[1]}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Phone:</strong> ${order[2]}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Address:</strong> ${order[3]}, ${order[4] || ''}, ${order[5] || ''}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Delivery Time:</strong> ${order[6] || 'N/A'}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Items:</strong><br>${(order[7] || '').replace(/\n/g, '<br>')}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Total:</strong> ₹${order[8]}
                        </div>
                        <div style="margin-bottom:10px;">
                            <strong>Status:</strong> ${order[13] || 'Pending'}
                        </div>
                    `;
                } else {
    ...
    async markDelivered() {
        ...
        try {
            ...
            if (data.success) {
                console.log('✅ Order delivered:', this.currentOrderId);
    ...
    // 🆕 GOOGLE MAPS NAVIGATION (Delivery Boy → Customer Route)
            ...
            if (data.success && data.orders) {
                ...
                if (order && order[10] && order[11]) {
                    ...
                    if (this.currentLocation) {
                        // Google Maps Directions: Delivery Boy → Customer
    ...
    initMap() {
        ...
        // Rider location marker (current location)
        if (this.currentLocation) {
